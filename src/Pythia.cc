...
bool Pythia::initPDFs() {
  ...
  if (settings.flag("PDF:useHard") && useNewPdfA && useNewPdfB) {
    ...
    pdfHardBPtr = getPDFPtr(idB, 3);      // @MM modified 2->3
...
PDF* Pythia::getPDFPtr(int idIn, int sequence) {
  ...
  if (abs(idIn) == 2212 && sequence == 1) {
    ...
    if (!useLHAPDF) {
      if (pSet == 66 || pSet == 67){ //user private PDFs
	tempPDFPtr = new MSTWpdf(idIn, pSet-2, xmlPath, &info);
      }
      else{ //user standard PDFs from Pythia
  ...
  // Proton beam, special choice for the hard process.
    ...
    // Use internal sets.
    if (!useLHAPDF) {
      if (pSet == 66 || pSet == 67 || pSet == 5){ //user private PDFs
	if (sequence == 2){
	  tempPDFPtr = new MSTWpdf(idIn, pSet-2, xmlPath, &info, 0);
	 
	}
	if (sequence == 3){
	  tempPDFPtr = new MSTWpdf(idIn, pSet-2, xmlPath, &info, 1);
