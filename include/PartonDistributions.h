...
public:
  ...
  MSTWpdf(int idBeamIn = 2212, int iFitIn = 1, string xmlPath = "../xmldoc/", 
	  Info* infoPtr = 0, int BeaM = -1) : PDF(idBeamIn) {init( iFitIn,  xmlPath, infoPtr, BeaM);}
...
private:
  ...
  static const double xxInit[65], qqInit[49];

  double xmin, xmax, qsqmin, qsqmax; //Added by MM
  ...
  void init( int iFitIn, string xmlPath, Info* infoPtr, int BeaM);
